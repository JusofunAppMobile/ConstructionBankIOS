...
@property (nonatomic ,strong) NSMutableDictionary *monthDataDic;
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if ([super initWithFrame:frame]) {
        ...
        _selectedDay = [NSString stringWithFormat:@"%d-%d-%d",(int)[_currentMonthDate dateYear],(int)[_currentMonthDate dateMonth],(int)[_currentMonthDate dateDay]];
...
- (void)setupCollectionViews {
    ...
    flowLayout.itemSize = CGSizeMake(self.bounds.size.width / 7.0, 39);
...
- (void)loadMonthDataArray{
    [_monthDataDic removeAllObjects];
    for (int i = 0; i<self.monthArray.count; i++) {
        GFCalendarMonth *info = self.monthArray[i];
        if ([info isKindOfClass:[GFCalendarMonth class]]) {
            NSString *str = [NSString stringWithFormat:@"%d-%d",(int)info.year,(int)info.month];
            [_datelist enumerateObjectsUsingBlock:^(CalendarMonthModel *obj, NSUInteger idx, BOOL * _Nonnull stop) {
                if ([str isEqualToString:obj.month]) {
                    [self.monthDataDic setObject:obj forKey:str];
                }
            }];
        }
    }
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
   ...
   __block CalendarDayModel *cellModel = nil;
    if (collectionView == _collectionViewL) {
        ...
        if (indexPath.row >= firstWeekday && indexPath.row < firstWeekday + totalDays) {
            ...
            cell.dayType = DayTypeThisMonth;
            if ((monthInfo.month == [[NSDate date] dateMonth]) && (monthInfo.year == [[NSDate date]  dateYear])) {
                if (indexPath.row == [[NSDate date]  dateDay] + firstWeekday - 1) {// 标识今天
                    cell.dayType = DayTypeToday;
                }
            }
            
            //赋值
            NSString *monthStr = [NSString stringWithFormat:@"%ld-%ld",monthInfo.year,monthInfo.month];
            CalendarMonthModel *monthModel = self.monthDataDic[monthStr];
            NSString *dayStr = [NSString stringWithFormat:@"%@-%@",monthStr,cell.todayLabel.text];
            [monthModel.date enumerateObjectsUsingBlock:^(CalendarDayModel *dayModel, NSUInteger idx, BOOL * _Nonnull stop) {
                if ([dayModel.day isEqualToString:dayStr]) {
                    cellModel = dayModel;
                }
            }];
            if ([_selectedDay isEqualToString:dayStr]) {
                cell.choosed = YES;
            }else{
                cell.choosed = NO;
            }
            ...
            cell.choosed = NO;
            cell.dayType = DayTypeOtherMonth;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.didSelectDayHandler != nil) {
        ...
        _selectedDay = [NSString stringWithFormat:@"%d-%d-%d",(int)year,(int)month,(int)day];
        [collectionView reloadData];
        self.didSelectDayHandler(year, month, day,cell.model); // 执行回调
    }
...
#pragma mark -
- (NSMutableDictionary *)monthDataDic{
    if (!_monthDataDic) {
        _monthDataDic = [NSMutableDictionary dictionary];
    }
    return _monthDataDic;
}
