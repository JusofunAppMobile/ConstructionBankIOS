...
- (NSMutableArray *)monthDataArray{
    if (!_monthDataArray) {
        _monthDataArray = [NSMutableArray arrayWithCapacity:4];
        [_datalist enumerateObjectsUsingBlock:^(CalendarMonthModel *obj, NSUInteger idx, BOOL * _Nonnull stop) {
            for (int i = 0; i<self.monthArray.count; i++) {
                GFCalendarMonth *info = self.monthArray[i];
                NSString *str = [NSString stringWithFormat:@"%li%02li",info.year,info.month];
                if ([str isEqualToString:obj.month]) {
                    [_monthDataArray addObject:obj];
                }
                if (_monthDataArray.count == _monthArray.count) {
                    *stop = YES;
                }
            }
        }];
...
- (void)setupCollectionViews {
    ...
    flowLayout.itemSize = CGSizeMake(self.bounds.size.width / 7.0, 78);
...
//        CalendarMonthModel *monthModel = self.monthDataArray[0];
        // 当前月
...
//                if ([dayModel.day isEqualToString:cell.todayLabel.text]) {
//                    cell.model = dayModel;
//                }
//            }];
